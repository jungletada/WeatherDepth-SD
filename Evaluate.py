"""
    ...
    Enhanced details:
        The code can be used to evaluate the model on the KITTI, DrivingStereo, and CADC datasets. 
        You can easily add your model at the func: create_model
        The code supports evaluation of the model with different baseline models, including 
            Monodepth2(ICCV2019), WaveletDepth(CVPR2021), MonoViT(3DV2022), and PlaneDepth(CVPR2023). 
        You can eaily add your model at the func: create_dataset
...
def batch_post_process_disparity_torch(l_disp, r_disp):
    ...
    l, _ = torch.meshgrid(torch.linspace(0, 1, w, device=l_disp.device), 
                          torch.linspace(0, 1, h, device=l_disp.device))
...
def create_dataset(opt):
    if opt.eval_split != 'stereo':
        ...
        # or you can set your drivingstereo dataset path here
        opt.data_path = opt.data_path.replace("kitti", "drivingstereo")
        # or you can change rain.txt to fog.txt to test the fog subsplit.
        filenames = readlines(os.path.join(splits_dir, opt.eval_split, "rain.txt"))
...
def inference(opt, dataset, dataloader, encoder, depth_decoder):
    ...
    i, start_idx, names = 0, 0, []  
    # name is use for drivingstereo dataset for it use the 'image with same name' to save the gt depth, 
    # ignore it if you don't use this dataset.
    ...
    with torch.no_grad():
        for data in tqdm(dataloader):
            ...
            if opt.post_process:
                ...
                pred_disp = batch_post_process_disparity_torch(pred_disp[:N], torch.flip(pred_disp[N:], dims=[2])) \
                    if opt.test_with_torch else batch_post_process_disparity(pred_disp[:N], pred_disp[N:, :, ::-1])
...
def print_title(name):
    ...
    title = ("&{:>10}" * 8).format(f"{name:>20}", "abs_rel", "sq_rel", "rmse", "rmse_log", "a1", "a2", "a3")
    title = title[1:]
    logger.info(title + "  \\\\")
...
def evaluate_all(opt):
    ...
    logging.info("-> Loading weights from {}".format(opt.load_weights_folder))
    ...
    if opt.eval_stereo:
        logging.info("   Stereo evaluation - "
    ...
    logging.info("-> Computing predictions with size {} x {}".format(opt.width, opt.height))
    # endregion
    ...
    if opt.test_with_weather:
        error_all = []
        load_val_mode = ['rgb/data', 'raingan/data', 'fog/150m', 
                         'snowgan/data', 'mix_rain/50mm', 'mix_snow/data', 'fog/75m']
        ...
        if not opt.disable_median_scaling:
            ...
            logging.info(" Scaling ratios | med: {:0.3f} | std: {:0.3f}".format(med, np.std(ratios / med)))
