...
from networks.model.lib_mamba.vmambanew import SS2D
...
class MobileMambaBlock(torch.nn.Module):
    def __init__(self, type, ed, global_ratio=0.25, local_ratio=0.25,
        ...
        if type == 's':
            self.mixer = Residual(MobileMambaBlockWindow(
                ed, 
...
class MobileMamba(torch.nn.Module):
    def __init__(self, img_size=224,
        ...
        for i, (stg, ed, dpth, gr, lr, do) in enumerate(
            ...
            for d in range(dpth):
                eval('self.blocks' + str(i + 1)).append(MobileMambaBlock(
                    stg, ed, gr, lr, kernels[i], dpr[d], ssm_ratio=ssm_ratio, forward_type=forward_type))
            if do[0] == 'subsample':
